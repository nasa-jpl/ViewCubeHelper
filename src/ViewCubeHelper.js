...
/**
 * @callback ViewCubeHelper~onLerpInitializeCallback
 * @param {Boolean} atTopOrBottomFace
 * @returns {void}
 */

/**
 * @callback ViewCubeHelper~onLerpCallback
 * @param {Number} lerpFactor
 * @returns {void}
 */

/**
 * @callback ViewCubeHelper~voidCallback
 * @returns {void}
 */

/**
 * @callback ViewCubeHelper~getOrbitDistanceCallback
 * @returns {Number}
 */
...
export default class ViewCubeHelper {
  ...
  constructor(camera) {
    ...
    this.maintainDistanceToOrbitCenter = true;
    ...
    this.defaultCameraDistance = 3.0;
    ...
    this.enforceMinimumCameraDistance = true;

    // the type of lerping that should be applied to the camera's orientation
    // see notes in doLerp() for more details on lerp types
    this.orientationLerpType = ViewCubeHelper.ORIENTATION_LERP_TYPE.LOOKAT;

    // set via setRollButtonType(), do not set directly
    this.rollButtonType = ViewCubeHelper.ROLL_BUTTON_TYPE.ONLY_AT_TOP_OR_BOTTOM_FACES;
    ...
    this.lerpTime = 1000.0; // 1.0 seconds

    // true if the user clicked on a face and subsequently only zoomed in/out or rolled the camera
    this.atFace = false;

    this.atTopOrBottomFace = false; // same as atFace but just for top or bottom faces
    this.onlyLerpingRoll = false; // true if we're only lerping camera roll (via the roll button)
  ...
  _setupViewCube() {
    ...
    this.VIEW_CUBE_CANVAS_WIDTH = 135;
    this.VIEW_CUBE_CANVAS_HEIGHT = 135;
    this._viewCubeRenderer.setSize( this.VIEW_CUBE_CANVAS_WIDTH, this.VIEW_CUBE_CANVAS_HEIGHT );
    ...
    // camera
    this.viewCubeCameraDistanceOffset = -500;
    this.viewCubeCamera = new THREE.OrthographicCamera(-1.5 * this.VIEW_CUBE_CANVAS_WIDTH, 1.5 * this.VIEW_CUBE_CANVAS_WIDTH,
      1.5 * this.VIEW_CUBE_CANVAS_HEIGHT, -1.5 * this.VIEW_CUBE_CANVAS_HEIGHT, 1, 2000);
    ...
    const recursiveAssignMaterial = (mat, node) => {
      if (node.geometry) node.material = mat;
      node.children.forEach(child => recursiveAssignMaterial(mat, child));
    }
    ...
    loader.load(CubeModelUrl, model => {
      this.viewCube = model.children[0];
      ...
      const hexColor = this.defaultColor.getHex();
      const red = ((hexColor & 0xff0000) >> 16) / 255.0;
      const green = ((hexColor & 0x00ff00) >> 8) / 255.0;
      const blue = ((hexColor & 0x0000ff)) / 255.0;

      for ( let i = 0; i < count; i++ ) {
        geometry.attributes.color.setXYZ( i, red, green, blue);//this.defaultColor.r, this.defaultColor.g, this.defaultColor.b );
      }
  ...
  setupMouseEvents() {
    ...
    this._viewCubeRenderer.domElement.addEventListener('mousedown', e => {
      this.viewCubeMouseDownPos = { x: e.pageX, y: e.pageY };
      this.viewCubeNeedToCastRay = false;
    });
  ...
  onClickFace(faceNormal) {
    ...
    if (!this.orientationLerpType !== ViewCubeHelper.ORIENTATION_LERP_TYPE.SLERP) {
    ...
    if (faceNormal.angleTo(up) < fiveDegreesInRad || faceNormal.angleTo(down) < fiveDegreesInRad) {
      if (!this.alwaysLerpUp && this.orientationLerpType === ViewCubeHelper.ORIENTATION_LERP_TYPE.LOOKAT) {
  ...
  setupRenderLoop() {
    ...
    const renderLoop = () => {
      ...
      this.viewCubeCamera.position.addScaledVector(viewCubeCameraDirection, this.viewCubeCameraDistanceOffset);
      ...
      if (cameraMoved) {
        ...
        if (this.atFace && !this.finishedLerpOnThisFrame && !this.lerping) {
          ...
          if (noLongerAtFace) {
            ...
            if (this.rollButtonType === ViewCubeHelper.ROLL_BUTTON_TYPE.ONLY_AT_FACES ||
              this.rollButtonType === ViewCubeHelper.ROLL_BUTTON_TYPE.ONLY_AT_TOP_OR_BOTTOM_FACES) {
  ...
  doLerp() {
    ...
    if (t - this.cameraStartLerpTime < this.lerpTime) {
      ...
      if (this.orientationLerpType === ViewCubeHelper.ORIENTATION_LERP_TYPE.SLERP || this.tempUseSlerp) {
        this.camera.quaternion.slerpQuaternions(this.cameraStartQuaternion, this.cameraEndQuaternion, lerpFactor);
        if (this.onLerping) this.onLerping(lerpFactor);
      } else if (this.orientationLerpType === ViewCubeHelper.ORIENTATION_LERP_TYPE.LOOKAT) {
        ...
        if (this.onLerping) this.onLerping(lerpFactor);
      } else { // this.orientationLerpType === ViewCubeHelper.ORIENTATION_LERP_TYPE.LERP
        this.lerpCameraUp.lerpVectors(this.cameraStartUp, this.cameraEndUp, lerpFactor);
        this.lerpCameraUp.normalize();
      ...
      if (this.rollButtonType === ViewCubeHelper.ROLL_BUTTON_TYPE.ALWAYS_ON ||
          this.rollButtonType === ViewCubeHelper.ROLL_BUTTON_TYPE.ONLY_AT_FACES ||
          (this.rollButtonType === ViewCubeHelper.ROLL_BUTTON_TYPE.ONLY_AT_TOP_OR_BOTTOM_FACES &&
  ...
  setupAxes() {
    ...
    this.yDomElement = this.xDomElement.cloneNode(true);
    this.yDomElement.innerText = 'Y';
    this.yDomElement.style.color = '#13FF00';

    this.zDomElement = this.xDomElement.cloneNode(true);
...
<path d="M16.377 18.7617V11.7617C16.377 8.44801 13.6907 5.76172 10.377 5.76172H5.87695" stroke="white" stroke-width="2" stroke-linecap="round"/>
    ...
    */
    ...
    this.svgMouseDownPos = { x: 0, y: 0 };
    svg.addEventListener('mousedown', e => {
      if (this.rollButtonDisabled || this.lerping) return;

      this.svgMouseDownPos = { x: e.pageX, y: e.pageY };
    });
    ...
    // default messages
    this.rollDisabledMsg = [];
    this.rollDisabledMsg[ViewCubeHelper.ROLL_BUTTON_TYPE.ALWAYS_ON] = '';
    this.rollDisabledMsg[ViewCubeHelper.ROLL_BUTTON_TYPE.ALWAYS_OFF] = 'Disabled';
    this.rollDisabledMsg[ViewCubeHelper.ROLL_BUTTON_TYPE.SHOW_DISABLED] = 'Disabled';
    this.rollDisabledMsg[ViewCubeHelper.ROLL_BUTTON_TYPE.ONLY_AT_FACES]
      = 'Click on Face to enable 90º roll button';
    this.rollDisabledMsg[ViewCubeHelper.ROLL_BUTTON_TYPE.ONLY_AT_TOP_OR_BOTTOM_FACES]
      = 'Click on Top or Bottom face to enable 90º roll button';
  ...
  setRollButtonType(type = null, disabledMsg = null) {
    ...
    if (this.rollButtonType === ViewCubeHelper.ROLL_BUTTON_TYPE.ALWAYS_ON)
      this.setRollButtonEnabled(true);
    else if (this.rollButtonType === ViewCubeHelper.ROLL_BUTTON_TYPE.ONLY_AT_FACES) {
      if (!this.atFace)
        this.setRollButtonEnabled(false, this.rollDisabledMsg[this.rollButtonType]);
    }
    else if (this.rollButtonType === ViewCubeHelper.ROLL_BUTTON_TYPE.ONLY_AT_TOP_OR_BOTTOM_FACES) {
    ...
    if (this.rollButtonType === ViewCubeHelper.ROLL_BUTTON_TYPE.ALWAYS_OFF)
      this.svg.style.display = 'none';
    else
      this.svg.style.display = 'block';
  }

  setRollButtonEnabled(enabled, disabledMsg = 'Disabled') {
  ...
  setRollButtonColors(enabled, hovered) {
    if (enabled) {
      if (hovered) {
  ...
  updateAxesLabels() {
    ...
    const overlapThreshold = 2 * 10.0 / this.VIEW_CUBE_CANVAS_WIDTH; // in NDC, set empirically after trial and error
    let hideX = false;
    let hideY = false;
    let hideZ = false;
    ...
    const ShowLabel = (element, pixel2D) => {
      element.style.display = "block";
      pixel2D.x = (( pixel2D.x * this.VIEW_CUBE_CANVAS_WIDTH ) + this.VIEW_CUBE_CANVAS_WIDTH) / 2.0;
      pixel2D.y = (( -pixel2D.y * this.VIEW_CUBE_CANVAS_HEIGHT ) + this.VIEW_CUBE_CANVAS_HEIGHT) / 2.0;
    ...
    if (hideX) {
      ...
      this.viewCubeRaycaster.setFromCamera(this.x2D, this.viewCubeCamera);
      ...
      if (labelIntersects.length > 0 && labelIntersects[0].distance < xDist) {
        ...
        ShowLabel(this.xDomElement, this.x2D);
      }
    }
    ...
    if (hideY) {
      ...
      this.viewCubeRaycaster.setFromCamera(this.y2D, this.viewCubeCamera);
      const labelIntersects = this.viewCubeRaycaster.intersectObject( this.viewCube, true );
      if (labelIntersects.length > 0 && labelIntersects[0].distance < yDist) {
        this.yDomElement.style.display = "none";
      } else {
        ShowLabel(this.yDomElement, this.y2D);
      }
    }
    ...
    if (hideZ) {
      ...
      this.viewCubeRaycaster.setFromCamera(this.z2D, this.viewCubeCamera);
      const labelIntersects = this.viewCubeRaycaster.intersectObject( this.viewCube, true );
      if (labelIntersects.length > 0 && labelIntersects[0].distance < zDist) {
        this.zDomElement.style.display = "none";
      } else {
        ShowLabel(this.zDomElement, this.z2D);
      }
    }
  }
  ...
  colorFaces(hexColor) {
    const colors = this.viewCube.geometry.attributes.color;
  ...
  colorFace(a, b, c, hexColor, colors) {
    const red = ((hexColor & 0xff0000) >> 16) / 255.0;
    const green = ((hexColor & 0x00ff00) >> 8) / 255.0;
    const blue = ((hexColor & 0x0000ff)) / 255.0;
  ...
  faceNormalsSame(index1, index2) {
    const normals = this.viewCube.geometry.attributes.normal;
    index1 *= 3;
    index2 *= 3;
   ...
   */
  setAxesOrientation(quatOrEuler) {
    if (quatOrEuler instanceof THREE.Quaternion) {
        this.axesGroup.quaternion.copy(quatOrEuler);
    } else {
        this.axesGroup.rotation.copy(quatOrEuler);
    }

    const tempPos = this.axes.position.clone().applyQuaternion(quatOrEuler);
    const labelAxesOffset = 35;
    this.x
        .set(AXES_LENGTH + labelAxesOffset, 0, 0)
        .applyQuaternion(quatOrEuler)
        .add(tempPos);
   ...
   * @param {(Quaternion|Euler)} quatOrEuler
   */
  setFaceOrientation(quatOrEuler) {
    if (quatOrEuler instanceof THREE.Quaternion) {
   ...
   * @return {void}
   */
  setTargetOrientation(quatOrEuler) {
    if (quatOrEuler instanceof THREE.Quaternion) {
...
ViewCubeHelper.ORIENTATION_LERP_TYPE = Object.freeze({
  SLERP: 'slerp',
  LOOKAT: 'lookat',
  LERP: 'lerp'
});

ViewCubeHelper.ROLL_BUTTON_TYPE = Object.freeze({
